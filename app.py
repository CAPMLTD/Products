...
<style>
  ...
  .risk-card:hover { border-color: #0a9b5c; background: #f0fdf4; }
  ...
  .risk-minimal      { border-left: 5px solid #0a9b5c; }
  ...
  .badge-optional  { background: #f0fdf4; color: #0a9b5c; }
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        d["model_name"]    = st.text_input("Model name *", value=d.get("model_name",""), placeholder="e.g. CustomerRisk-Classifier-v2")
        d["model_type"]    = st.text_input("Model type / architecture *", value=d.get("model_type",""), placeholder="e.g. Transformer-based classifier / CNN / YOLO + ResNet head")
        d["developers"]    = st.text_input("Developers / Organisation *", value=d.get("developers",""), placeholder="e.g. Acme AI Ltd")
    with col2:
    ...
    with col1:
        ...
        d["data_size"]     = st.text_input("Data size / volume *", value=d.get("data_size",""), placeholder="e.g. 50,000 labelled samples across 10 deployment sites")
        d["date_range"]    = st.text_input("Date range of data", value=d.get("date_range",""), placeholder="e.g. Jan 2023 – Dec 2025")
    with col2:
        ...
        d["known_biases"]  = st.text_area("Known data biases or gaps *", value=d.get("known_biases",""), height=100,
            placeholder="Class imbalance, geographic skew, seen/unseen environment distribution, lighting or condition variation.")
        d["demographic_notes"] = st.text_area("Demographic representation notes", value=d.get("demographic_notes",""), height=60,
            placeholder="Geography, user demographics, device types, environmental conditions covered.")
    section_end()
    ...
    with col1:
        d["eval_datasets"]  = st.text_area("Evaluation datasets *", value=d.get("eval_datasets",""), height=100,
            placeholder="Name each split or benchmark. Include seen vs unseen environment breakdown where applicable.")
        d["metrics_used"]   = st.text_input("Metrics used *", value=d.get("metrics_used",""), placeholder="e.g. Precision, Recall, F1, AUC-ROC, True Positive Rate")
        d["results_summary"] = st.text_area("Results summary *", value=d.get("results_summary",""), height=100,
            placeholder="Key numbers per metric per split. e.g. Test set F1: 0.87, Held-out recall: 0.79.")
    with col2:
        d["disaggregated_eval"]    = st.text_area("Disaggregated evaluation (subgroups)", value=d.get("disaggregated_eval",""), height=100,
            placeholder="Performance broken down by demographic group, geography, device type, or data slice.")
        d["benchmark_comparisons"] = st.text_area("Benchmark / competitor comparisons", value=d.get("benchmark_comparisons",""), height=60,
            placeholder="Comparison to baseline model, industry benchmark, or prior version.")
    section_end()
    ...
    with col1:
        ...
        d["failure_modes"]         = st.text_area("Failure modes", value=d.get("failure_modes",""), height=80,
            placeholder="Known edge cases: occlusion, low signal, novel input types, out-of-distribution data.")
    with col2:
        d["fairness_concerns"]    = st.text_area("Fairness / bias concerns *", value=d.get("fairness_concerns",""), height=100,
            placeholder="Disparate performance across demographic groups, geographies, or input conditions.")
        d["conditions_degrade"]   = st.text_area("Conditions where performance degrades", value=d.get("conditions_degrade",""), height=80,
            placeholder="Domain shift, adversarial inputs, novel environments, low-resource scenarios.")
    section_end()
    ...
    with col1:
        d["primary_use_cases"]       = st.text_area("Primary use cases *", value=d.get("primary_use_cases",""), height=100,
            placeholder="Specific tasks and scenarios this model is designed for. e.g. Fraud detection in financial transactions.")
        d["intended_users"]          = st.text_area("Intended users / audience", value=d.get("intended_users",""), height=80,
            placeholder="Risk analysts, operations teams, compliance officers, system integrators.")
    with col2:
...
{html_body}
<div class="footer">Generated by CA Project Management Services Ltd — EU AI Act Model Card Generator — <a href="https://www.caprojectmgmt.com" style="color:#0a9b5c;">caprojectmgmt.com</a></div>
</body>
</html>"""
        st.download_button(
            "⬇️ Download HTML",
    ...
    with col3:
        try:
            ...
            # Footer
            doc.add_paragraph()
            footer_para = doc.add_paragraph("Generated by CA Project Management Services Ltd — EU AI Act Model Card Generator — caprojectmgmt.com")
    ...
    st.markdown("---")
    st.markdown(
        "<div style='text-align:center;color:#9ca3af;font-size:0.8rem;'>CA Project Management Services Ltd &nbsp;·&nbsp; EU AI Act Model Card Generator &nbsp;·&nbsp; <a href='https://www.caprojectmgmt.com' target='_blank' style='color:#0a9b5c;text-decoration:none;'>caprojectmgmt.com</a> &nbsp;·&nbsp; Powered by Claude</div>",
        unsafe_allow_html=True,
    )
